...
‌<br/><br/>
مسئله من در این مقاله معرفی تکنیک‌ها و روش‌هایی برای شیوۀ نگارش مطالب نیست. در [[efficient note taking|تکنیک‌های یادداشت‌برداری کارآمد]] مفصل در این مورد توضیح دادم و اشاره کردم که با استفاده از تکنیک‌هایی مثل نقشه‌های ذهنی یا مفهومی، اسکچ نوت یا یادداشت بصری، لایه‌لایه کردن مطلب و... می‌توانید فرم و محتوای یادداشت‌تان را بهبود ببخشید.
<br/><br/>
...
> 
> اگر از [[obsidian review|نرم افزار ابسیدین(Obsidian)]] استفاده کنید به راحتی می‌توانید از بخش backlink و [[obsidian review#گراف (Graph view)|Graph view]] محتوای یتیم را شناسایی کنید. به‌عنوان نمونه به همین سایت که با کمک ابسیدین و [کوارتز](https://quartz.jzhao.xyz/) ساخته شده نگاه کنید. بخش «نمای گراف» را باز کنید. نقطه‌هایی که به اطراف پراکنده شدند و هیچ اتصالی با سایر نقاط ندارند محتوای یتیم هستند. در بخش «بک‌لینک» نیز فهرست نوشته‌ها نشان می‌دهد که این یادداشت محتوای یتیم نیست چون از صفحات مختلفی به آن ارجاع داده شده است.
> <br/><br/>
...
> - **مرحله انجام:** وضعیت انجام و تکمیل یادداشت مانند `⬜پیش نویس` - `🟨درحال انجام` - `🟩تکمیل شده` - `🟥کنسل شده`.<br/><br/>
...
همانطور که در مرحله اول (اماده سازی زمین) منابع مصرفی شما محدود به کتاب نبود و همه چیز مثل پادکست، ویدئو، دوره، وبینار، فیلم و مستند را ثبت و ضبط کردید، در مرحله دوم (کاشت) هم از فرم‌های مختلف استفاده کنید.
<br/><br/>
### ۵. مستقل باشید
...
روم ریسرچ نیز مانند ابسیدین بر اساس لینک‌های داخلی طراحی شده است. این ابزار به شما امکان می‌دهد تا به راحتی یادداشت‌ها و اطلاعات خود را به یکدیگر مرتبط کنید و یک شبکه اطلاعاتی پویا بسازید.
<br/><br/>
### ابزار‌های دیگر
